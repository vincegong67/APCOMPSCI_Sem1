...
public class Picture extends SimplePicture 
{
  ...
  public void mirrorArms()
  {
    ...
    for (int row = 166; row < 198; row++)
    {
      ...
      {
        topPixel = pixels[row][col];
        bottomPixel = pixels[row + 246-178][col];
        bottomPixel.setColor(topPixel.getColor());
		count++;
      }
    }
	...
	int mirrorPointtwo = 173;
    
    // loop through the rows
    for (int row = 158; row < 192; row++)
    {
      // loop from 13 to just before the mirror point
      for (int col = 110; col < mirrorPointtwo; col++)
      {
        topPixel = pixels[row][col];
        bottomPixel = pixels[row + 246-178][col];
        bottomPixel.setColor(topPixel.getColor());
  ...
  public void edgeDetection(int edgeDist)
  {
	...
	// Create 2 pixel objects
	// get array of pixels from the picture
	// loop through the entire picture
	// for each pixel	
		// check against the next pixel
		// if the color distance between the 2 pixels is 
		// greater than the set edgeDist
